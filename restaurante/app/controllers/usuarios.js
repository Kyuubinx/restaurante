...
module.exports.carrinho = async function (app, req, res){
    ...
    const idPedido = modelPedido.getIdPedidoAberto(req.session.id_usuario)
    const carrinho = modelCarrinho.getCarrinho(idPedido)

    let valortotal;

    for (i = 0; i < carrinho.length; i++){
        carrinho[i].produto = modelProduto.getProdutoById(carrinho[i].id_produto)

        valortotal = carrinho[i].quantidade * carrinho[i]["produtos"].preco 
